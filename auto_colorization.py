...
from batchnorm import ConvolutionalBetchNormalizer
...
def read_my_file_format(filename_queue, randomize=False):
    """ This function read a list of filename from queue, Then convert the
    image into float point format. """
    reader = tf.WholeFileReader()
    key, file = reader.read(filename_queue)
    uint8image = tf.image.decode_jpeg(file, channels=3)
    
    # todo: Crop? or Resize using a "neural network"?
    uint8image = tf.random_crop(uint8image, (224, 224, 3))
    
    # Since position is important to neural network, flip is not a good idea
    if randomize:
...
def input_pipeline(filenames, batch_size, num_epochs=None):
    ...
    example = read_my_file_format(filename_queue, randomize=False) # Get float Image
...
def batch_norm(x, depth, phase_train):
    """ Construct a batch norm of some node x """
    with tf.variable_scope('batchnorm'):
        ewma = tf.train.ExponentialMovingAverage(decay=0.9999)
        bn = ConvolutionalBetchNormalizer(depth, 0.001, ewma, True)
...
def colornet(_tensors):
    with tf.variable_scope('colornet'):
        # B x 28 x 28 x 512 -> batchnorm -> 1x1 conv = Bx28x28x256
        conv1 = tf.nn.relu(tf.nn.conv2d(
            batch_norm(_tensors['conv4_3'], 512, phase_train), _tensors['weights']['wc1'], [1, 1, 1, 1], 'SAME'
        ))
        # upscale to 56x56x256
        conv1 = tf.image.resize_bilinear(conv1, (56, 56))
        conv1 = tf.add(conv1, batch_norm(_tensors['conv3_3'], 256, phase_train))
        ...
        conv2 = tf.add(conv2, batch_norm(_tensors['conv2_2'], 128, phase_train))
        ...
        conv4 = tf.add(conv4, batch_norm(_tensors['grayscale'], 3, phase_train))
        ...
        conv6 = conv2d(conv5, _tensors['weights']['wc6'], sigmoid=False, bn=True)
...
def main():
    ...
    pred_yuv = tf.concat([tf.split(grayscale_yuv, num_or_size_splits=3, axis=3)[0], pred], axis=3)
    pred_rgb = yuv2rgb(pred_yuv)
    
    loss = tf.square(tf.subtract(pred, tf.concat(
        axis=3, values=[tf.split(colorimage_yuv, num_or_size_splits=3, axis=3)[1], tf.split(colorimage_yuv, num_or_size_splits=3, axis=3)[2]]
    )))
    
    if uv == 1:
        loss = tf.split(loss, axis=3, num_or_size_splits=2 )[0]
    elif uv == 2:
        loss = tf.split(loss, axis=3, num_or_size_splits=2 )[1]
    else:
        loss = (tf.split(loss, axis=3, num_or_size_splits=2 )[0] + tf.split(loss, axis=3, num_or_size_splits=2)[1]) / 2
    ...
    tf.summary.image("grayscale_rgb", grayscale_rgb, max_outputs=1)
    ...
    writer = tf.summary.FileWriter('colornet_log/', sess.graph)
