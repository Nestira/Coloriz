...
ROOT_DIR = os.getcwd()
MASK_LOGS_DIR = os.path.join(ROOT_DIR, 'Mask_auto_color_fixedsize_logs')
COCO_MODEL_PATH = os.path.join(ROOT_DIR, 'mask_rcnn_coco.h5')

TB_LOG_DIR = os.path.join(ROOT_DIR, 'auto_color_tb_logs')
TESTING_RESULT_DIR = os.path.join(ROOT_DIR, 'auto_color_mrcnn_test')
...
def main():
    ...
    if os.path.isfile('color_tensorflow_real_mode.h5'):
        print('Found weights')
    ...
    # for i in range(30):
        ...
        if i % 10 == 0:
            report_loss()
            colored = colorize()
            skimage.io.imsave(os.path.join(TESTING_RESULT_DIR, '{}_test_'.format(i) + "00000813.jpg"),arr=colored)
        
        if i % 500 == 499:
            model.save_weights("{}_color_mrcnn.h5".format(i))
    ...
    model.save_weights("color_tensorflow_final.h5")
